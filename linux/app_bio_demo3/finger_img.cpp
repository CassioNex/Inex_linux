...
FingerImg::FingerImg(QWidget *parent) : QWidget(parent)
{
    ...
    */
    ...
    // Timer para medição do tempo de execução das funções
...
void FingerImg::handleImgCaptured(uint8_t *buf)
{
    ...
    */
    // Resultado de score do search
    int search_res;
    // Resultados do match, guardando o maior do banco em match_res_n1
    int match_res;
    ...
    */
    ...
    // Search mode
        ...
        if (create_template((BYTE*)/*img_buf*/buf, N_COL, N_LIN, (BYTE*)pfeature_search) == 1) {
           ...
           search_res = finger_search((BYTE *)pfeature_search, (BYTE *)pfeature_db, N_TEMPLATES, MEDIUM_LEVEL, &match_res_n1);
    #else
           search_res = finger_search((BYTE *)pfeature_search, (BYTE *)pfeature_db, N_TEMPLATES, MEDIUM_LEVEL, (BYTE *)/*img_buf*/buf);
    #endif
           qDebug() << "finger_search() levou " << timer->elapsed() << "milliseconds";
#else
