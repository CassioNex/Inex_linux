...
void get_segmentation(BYTE *pImg,BYTE* OrntImg,int cxDIB,int cyDIB)
{
	...
	int *varHist, xHist[MAX_IMG_WIDTH/4+1], yHist[MAX_IMG_HEIGHT/4+1];
...
{
	BYTE *image_buffer3 = NULL, *image_buffer4 = NULL, *image_buffer2 = NULL, *image_buffer5 = NULL;
	...
	image_buffer4 = image_buffer2 + cxDIB*cyDIB;
	...
	image_buffer5 = (BYTE*)calloc(cxDIB*cyDIB, sizeof(BYTE));
	if (image_buffer5 == NULL)  return ERR_CAN_NOT_ALLOC_MEMORY;
	memcpy(image_buffer5, image_buffer4, sizeof(BYTE)*cxDIB*cyDIB);
	image_buffer1 = (USHORT*)image_buffer2;
	...
	get_point_value(&tempVect,image_buffer5,cxDIB,cyDIB);

	arrange_mp(&tempVect,&FPEx.Mp,image_buffer3,cxDIB,cyDIB);

	free(image_buffer5);  //free(image_buffer2);  free(image_buffer1); free(image_buffer3); 
...
void shift_points(int* nXoffset,int* nYoffset,LPFPVECTEX pVect,int* XField,int* YField)
{
	...
	*nXoffset = ( nSum2 == 0 ) ? 0 : (nSum1/nSum2) - 320;
	...
	memcpy ( YField, pTmp, sizeof(short)*640 );
	nMax = nMaxId = 0;
	for (i = 0; i < 640; i++) {
...
int point_matching(LPFPVECTEX pFile,LPFPVECTEX pSearch,PAIRVECT *pPair,BOOL nCollectFlag,BOOL nCommonFlag,int nScoreTh,int *nGlobalScore,BYTE *img) 
{
	/*BARVECT FileBar, SearchBar;
	PAIRBAR PairList[1000], PairTemp[200];
	int AngleField[240], SDiffField[240], FDiffField[240];
	int XField[640], YField[640], LastList[300];
	int SArrangBarPtr[240*MAX_SAMEDIFF_NUM], FArrangBarPtr[240*MAX_SAMEDIFF_NUM];*/

	BARVECT *FileBar, *SearchBar;
	PAIRBAR *PairList, *PairTemp;
	int *AngleField, *SDiffField, *FDiffField;
	int *XField, *YField, *LastList;
	int *SArrangBarPtr, *FArrangBarPtr;
	BAR FBar, SBar;
	...
	AngleField = (int*)img;
	SDiffField = AngleField + 240;
	FDiffField = SDiffField + 240;
	XField = FDiffField + 240;
	YField = XField + 640;
	LastList = YField + 640;
	SArrangBarPtr = LastList + 500;
	FArrangBarPtr = SArrangBarPtr + 240 * MAX_SAMEDIFF_NUM;
	FileBar = (BARVECT *)(FArrangBarPtr + 240 * MAX_SAMEDIFF_NUM);
	SearchBar = FileBar + 1;
	PairList = (PAIRBAR *)(SearchBar + 1);
	PairTemp = PairList + 1000;
	...
	score_sum = adjust_score(score_sum,score,nBlockScore,nCommonScore,dscore,
		...
		if (pFile->Mp.quality < 32 && pSearch->Mp.quality < 32)	{
				...
				score_sum = score_sum*nBlockScore*nBlockScore/2000;
		}
...
int matching_main(LPFPFEATURE pFeatureVect1,LPFPFEATURE pFeatureVect2,int securitylevel,BYTE *img)
{
	...
	score = point_matching(&Vect1,&Vect2,&pPair,FALSE,TRUE,nTh,&nGlobalScore,img);
	if (score < nTh) {
		Vect1 = TmpVect1; Vect2 = TmpVect2;
		score = point_matching(&Vect2,&Vect1,&pPair,FALSE,TRUE,nTh,&nGlobalScore,img);
		if (score < nTh) {
 ...
 */
int finger_match(BYTE* pFeature1,BYTE* pFeature2,int securitylevel,BYTE* img)
{
	int res;
	...
	res = matching_main(pVect1,pVect2,securitylevel,img);
	/* O retorno da função matching_main é 1 ou -1!!!
	//if (res < 0) res = 0;
	//if (res > 1000) res = 1000;

	if ( res == 1 ) return ERR_OK;
 ...
 */
int finger_search(BYTE* pFeature,BYTE* pDBFeature,int nDBSize,int securitylevel,BYTE* img)
{
	int i, idx, res, nMin;
	short *pIndexArray;
